...
static void OnTick()
{
    ...
    if (!IS_PED_DEAD_OR_DYING(playerPed, false) && IS_PED_IN_ANY_VEHICLE(playerPed, false) && GET_IS_VEHICLE_ENGINE_RUNNING(veh))
    {
        ...
        {
            sprintf_s(charBuf, "Minimo: %s\nDetonacion En: %.1fs", displayMinSpeed.c_str(), float(m_timeReserve) / 1000);
        }
        else if (m_timeReserve != WAIT_TIME && speedms > minSpeed)
        {
            sprintf_s(charBuf, "Minimo: %s\nDetonacion En: %.1fs (Recargando)", displayMinSpeed.c_str(), float(m_timeReserve) / 1000);
        }
        else
        {
            sprintf_s(charBuf, "Minimo: %s", displayMinSpeed.c_str());
        }
        SCALEFORM_MOVIE_METHOD_ADD_PARAM_PLAYER_NAME_STRING(charBuf);
